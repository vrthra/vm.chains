...
# get sources
if [ -f /vagrant/chains/toolchains.tar.gz ];
then
  echo Exists
  mkdir -p "${SRC_DIR}"
  (cd ~/ && zcat /vagrant/chains/toolchains.tar.gz | tar -xpf - )
else
  echo Doesnt Exist
  mkdir -p "${SRC_DIR}"
  ...
  tar -cf /vagrant/chains/toolchains.tar toolchains/build/
  gzip /vagrant/chains/toolchains.tar
fi
#
